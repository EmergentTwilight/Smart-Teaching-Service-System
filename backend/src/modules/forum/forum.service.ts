...
const inferAttachmentMimeType = (fileName: string, mimeType?: string): string => {
...
export class ForumService {
  ...
  private static validateAttachment(file: UploadedFile): void {
    ...
    const mimeType = inferAttachmentMimeType(file.originalname, file.mimetype);
  ...
  static async getPostDetail(postId: string, userId: string) {
    ...
    const hasAccess =
      ...
      (await this.isAdminOrTeacher(userId)) ||
      (await this.verifyCourseAccess(userId, post.courseOfferingId));
    if (!hasAccess) {
  ...
  static async searchPosts(query: SearchPostsDto, userId: string) {
    ...
    const posts = matchedPosts.slice((page - 1) * pageSize, page * pageSize);
