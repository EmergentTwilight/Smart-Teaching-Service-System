...
| `frontend/src/modules/course-arrangement/pages/classroom-edit.tsx`              | 教室状态枚举拼写错误                                  | `MAINTENENCE` 改为 `MAINTENANCE`                                       | 已完成页面回归                           |
| `frontend/src/modules/course-arrangement/pages/classroom-edit.tsx`              | 教室表单设备字段是扁平结构，提交后设备信息不会落库    | 改为提交嵌套 `equipment` 字段                                          | 已完成页面回归                           |
...
| 编号         | 问题                                                  | 处理                                                                                             |
| ------------ | ----------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `B-AUTH-002` | 排课写接口只有登录校验，学生/教师登录后也可调用写操作 | `classroom`、`schedule`、`rule`、`auto-schedule` 写接口加 `requireRoles('admin', 'super_admin')` |

### 已验证

| 内容           | 结果 | 说明                                           |
| -------------- | ---- | ---------------------------------------------- |
| 写接口鉴权测试 | 通过 | 4 个测试文件、44 条用例通过                    |
| 后端类型检查   | 通过 | `pnpm --filter @stss/server typecheck` 通过    |
| 读接口回归     | 通过 | 教室列表读取仍允许普通登录用户访问，未被误收紧 |
