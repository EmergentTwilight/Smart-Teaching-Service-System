...
- 培养方案查看、制定与确认：展示学生所属专业培养方案、课程分类、总学分、必修/选修学分要求和建议修读学期，支持学生据此制定并确认选课依据；确认结果作为选课流程的前置条件。
...
| `Curriculum` | 读取专业培养方案、适用年级、总学分、必修学分和选修学分要求；不要求独立公共课最低学分字段。 | A、C、F |
...
| 服务接口 | C 子系统服务 | C 子系统应提供培养方案查看与确认、课程搜索、课程详情、可选课程列表、选课、退选、选课结果查询、名单导出、选课时间段管理、手动加课和 AI 辅助建议能力。 |
...
| ---- | -------- | ---------- | -------- | ---------- | ---------- |
| `US-C-01` | 学生制定并确认培养方案 | 学生 | 学生已登录；学生所属专业和年级存在对应培养方案 | 系统展示培养方案中的课程分类、学分要求和建议修读学期；学生确认后，系统记录确认结果，作为进入选课流程的资格依据 | 缺少培养方案、存在多个匹配方案或确认信息不完整时，应提示无法进入正式选课流程并联系教务管理人员。 |
...
| `US-C-04` | 系统自动显示可选课程 | 学生 | 学生所属专业和年级存在可匹配培养方案；学生已确认当前培养方案；当前存在启用的选课时间段 | 系统根据培养方案、课程开设状态、容量和选课阶段生成可选课程列表 | 若未确认培养方案或无可选课程，系统应显示原因而不是空白页面。 |
| `US-C-05` | 学生完成选课 | 学生 | 当前处于选课开放时间；学生已确认当前培养方案；目标课程容量未满；无冲突 | 学生点击选课，系统校验所有约束后创建 `enrolled` 状态的选课记录，并更新已选人数 | 未确认培养方案、容量满、重复选课、时间冲突、超过最大学分、不在选课时间段或先修课未通过时应拒绝。 |
...
| `US-C-09` | 教务管理人员控制选课时间段 | 教务管理人员 | 教务管理人员已登录并具备管理权限 | 管理人员设置初选、补退选或调整阶段的开始时间、结束时间、最大学分、是否允许退课和启用状态 | 时间范围非法、同一学期阶段冲突或缺少学期信息时应拒绝保存。 |
...
培养方案制定功能用于支持学生在选课前理解所属专业的学分要求和课程结构，并据此确认当前培养方案作为正式选课依据。系统应以培养方案、培养方案课程和课程类型为基础展示专业必修课、专业选修课和公共课等课程类别，不应由 C 模块自行定义与 A 模块冲突的课程主数据。v2.0 验收基线要求培养方案确认结果可供选课流程校验；当前数据库设计尚未包含学生培养方案确认记录，需先完成数据库设计待办后再实现持久化确认和阻断校验。公共课仅按课程类型统计已选学分。
...
| `FR-C-04` | 学生进入正式选课前，系统应提供培养方案查看、制定与确认入口；确认结果应可被选课流程读取，作为学生进入正式选课的前置条件。 | Must |
| `FR-C-05` | 系统应根据已确认培养方案、本人有效选课记录和课程学分统计学生当前选课计划的学分构成，包括总学分、必修课学分、选修课学分和公共课已选学分；公共课不设置独立最低学分达标判断。 | Must |
...
| 编号 | 功能需求 | 优先级 |
| ---- | -------- | ------ |
| `FR-C-13` | 系统应根据登录学生已确认培养方案自动显示可选课程列表，并标识课程是否未确认培养方案、已选、容量已满、时间冲突、符合培养方案和满足先修要求。 | Must |
| `FR-C-14` | 系统应仅允许学生在启用且当前时间位于起止时间范围内的 `SelectionPeriod` 中执行选课或退选操作。 | Must |
| `FR-C-15` | 可选课程列表应展示主要课程信息、当前选课人数和课程容量。 | Must |
| `FR-C-16` | 学生提交选课时，系统应校验培养方案确认状态、课程开设状态、课程容量、重复选课、选课时间段、最大学分、培养方案适配性、课表冲突和先修课通过状态。 | Must |
...
| `FR-C-19` | 系统应基于 `CoursePrerequisite` 判断目标课程是否存在先修课程要求，并依据学生有效成绩或等价课程完成记录判断是否通过；未通过或缺少可判定数据时应阻止选课并给出明确提示。 | Must |
...
| `FR-C-35` | 系统应提供选课准入控制能力，限制同时进入选课系统核心流程的活跃用户数；达到上限时应拒绝或返回排队/稍后重试提示，防止系统过载导致无法服务。 | Must |
| `FR-C-36` | 系统应能够对长时间停留或长时间无有效操作的用户执行强制退出或释放选课会话名额，释放后用户可重新进入，且不得影响已成功保存的选课记录。 | Must |
...
flowchart TB
    ...
    student -- "培养方案查看 / 搜索课程 / 选课退选 / AI 请求" --> C
...
| `Curriculum` | 表示专业培养方案；提供总学分、必修学分、选修学分和适用年级等约束，不提供公共课独立最低学分约束。 | `Student`、`CurriculumCourse` |
...
| `SelectionPeriod` | 表示选课时间段；控制初选、补退选和调整阶段，以及最大学分、是否允许退课和启用状态。 | `Enrollment`、教务管理人员 |
...
| `NFR-C-07` | 约束可解释性 | 选课失败时，系统应明确提示失败原因，如未确认培养方案、容量已满、时间冲突、超过最大学分、不在选课时间段或先修课未通过。 |
...
| 编号 | 覆盖需求 | 验证标准 |
| ---- | -------- | -------- |
| `VC-C-01` | `FR-C-01` 至 `FR-C-07` | 使用具有专业和年级信息的学生账号进入培养方案页面，系统能展示对应培养方案、学分要求、课程分类和建议修读学期；学生确认后具备进入正式选课流程的资格；公共课只验证已选学分统计，不验证独立最低学分目标；缺少培养方案时给出明确提示并阻止生成可选课程列表。 |
| `VC-C-02` | `FR-C-08` 至 `FR-C-12` | 按课程名称和教师搜索课程时，返回结果与条件一致；课程详情包含学分、类型、教师、容量、已选人数、先修要求、排课安排和考核方式。 |
| `VC-C-03` | `FR-C-13`、`FR-C-15` | 学生进入可选课程页面时，系统根据已确认培养方案和课程开设自动显示可选课程，并展示课程信息、选课人数和容量；未确认培养方案时返回明确不可选原因。 |
...
| `VC-C-07` | `FR-C-16`、`FR-C-19` | 对配置了先修课程的目标课程，系统依据有效成绩或等价课程完成记录判断通过状态；未通过或缺少可判定数据时阻止选课并返回明确提示。 |
...
| `VC-C-13` | `FR-C-35`、`NFR-C-01`、`NFR-C-02` | 使用至少 200 个并发在线用户模拟访问选课核心流程，系统保持可用；超过选课准入阈值的请求返回等待或稍后重试提示，但不影响已进入用户的选课一致性。 |
| `VC-C-14` | `FR-C-36`、`NFR-C-03` | 模拟用户长时间无操作，系统释放选课会话名额或要求重新进入；释放后用户可重新进入，且此前已成功保存的选课记录不被删除。 |
...
> 说明：`d7de14ce` 对应的 v2.0 总需求报告使用压缩版 C 组编号（`FR-C-01` 至 `FR-C-16`）。本 C 组详细 SRS 保留既有 `FR-C-01` 至 `FR-C-43` 编号体系；其中 v2.0 `FR-C-15 选课连接控制与空闲释放` 对应本文 `FR-C-35`、`FR-C-36`、`NFR-C-01` 至 `NFR-C-03`、`VC-C-13` 和 `VC-C-14`。
