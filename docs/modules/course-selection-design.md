...
| 教师查看名单 | 只能查看本人任课课程；`academic_admin` 如需查看必须由 API 文档明确授权。 |
| 教务管理阶段 | 仅 `academic_admin` 可操作；`system_admin` 不默认替代教务权限。 |
| 手动加课 | 仅 `academic_admin` 可操作，并记录原因；`system_admin` 不默认替代教务权限。 |
| AI 辅助 | 仅使用当前用户可见数据生成建议。 |

具体角色名应与 C 组 API/SRS 保持一致，学生端使用 `student`，教师端使用 `teacher`，教务管理端使用 `academic_admin`。如仓库已有 `system_admin`，不得默认视为教务管理权限，必须通过明确授权获得 `academic_admin` 等价能力。
