...
| 类型 | 内容 |
|---|---|
| 范围内 | 培养方案查看与确认、学分进展、课程搜索、课程详情、可选课程列表、学生选课/退选、选课结果、个人课表、教师名单导出、选课阶段管理、手动加课、选课准入控制、长时间无操作释放、AI 辅助选课。 |
...
| 子模块 | 对应功能域 | 说明 |
|---|---|---|
| C1 培养方案与学分进展 | 培养方案查看与确认 | 展示学生所属培养方案、课程类别、建议学期、总学分和分类学分进度；提供培养方案确认入口和确认状态展示。 |
| C2 课程搜索与详情 | 课程搜索与查看 | 按课程名称、课程代码、教师、学期、课程类型等条件搜索课程开设，展示容量、已选人数和可选原因。 |
| C3 选课/退选核心流程 | 选课功能 | 实现选课事务、退选事务、培养方案确认校验、容量控制、重复选课、课表冲突、最大学分、先修课检查和选课核心流程准入控制。 |
| C4 结果、课表与名单导出 | 结果查询 | 学生查看本人选课结果和课表；教师查询和导出本人课程学生名单。 |
| C5 选课管理与并发控制 | 选课管理 | 管理初选、补退选、调整阶段；支持手动加课；协作提供选课准入控制和长时间无操作释放的阶段、配置和管理口径。 |
...
```text
1. 当前用户必须是 student。
2. 当前学生必须已确认当前匹配培养方案；确认状态必须来自后端持久化记录。
3. 当前必须存在启用的 SelectionPeriod，且服务端当前时间在 [start_time, end_time] 内。
4. CourseOffering 必须处于可选状态。
5. CourseOffering.enrolled_count 必须小于 CourseOffering.capacity。
6. 当前学生不得对同一 CourseOffering 存在有效重复选课记录。
7. 目标课程 Schedule 不得与当前学生已选课程 Schedule 冲突。
8. 当前阶段选课总学分不得超过 SelectionPeriod.max_credits。
9. 若存在 CoursePrerequisite，必须依据有效成绩或等价课程完成记录检查先修课是否满足；若暂未接入 F 组数据，应拒绝并保留 TODO 和错误边界。
10. 创建或更新 Enrollment，并与 CourseOffering.enrolled_count 的增加在同一事务中完成。
```
...
AI 进度画像以 F 组 `Score` 中 `SUBMITTED`/`CONFIRMED` 且达到及格线的有效成绩作为已完成课程；当前学期 `ENROLLED` 课程只作为在修学分、课表冲突和阶段学分上限判断依据。

学生可以将某次 AI 推荐或单门课程解释保存为个人快照，用于刷新页面后回看。保存快照只记录生成当时的请求摘要和结果，不作为正式选课依据；从保存记录继续选课时，后端必须重新执行正式选课校验。保存、查看和删除快照均只能操作当前登录学生本人的数据，不接收前端传入 `student_id`。

### 8.6 并发与超时

系统应支持至少 200 名在线用户同时使用选课服务。v2.0 总报告 `FR-C-15` 的最优归属为 C3 主责、C5 协作：C3 负责在选课核心流程接入准入控制、心跳和长时间无操作释放；C5 负责阶段、管理配置和教务侧协作口径。该能力不得依赖新增数据库业务表，释放会话不得影响已成功保存的选课记录。
