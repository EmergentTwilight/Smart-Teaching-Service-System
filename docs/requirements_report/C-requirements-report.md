...
> 本文只覆盖 C 组“智能选课”子系统，并严格按当前代码实现和可见测试描述需求口径。AI 推荐后端已接入规则过滤与 LLM 推荐，包含降级到模板解释的能力；Redis 连接控制、先修课成绩通过判断、培养方案确认持久化和 200 在线用户压测仍未完成，本文不将其写成已实现能力。
...
| 当前未实现项 | Redis 连接控制和无操作释放为 TODO；`ai-advisor.service` 已接入规则过滤+LLM 并支持降级 |
...
| AI 辅助选课 | 已接入（含降级） | 后端返回规则基础建议；LLM 可用时返回推荐解释，不可用时回退规则模板；不产生选课副作用 |
...
| 编号 | 问题 | 影响范围 | 状态 |
|---|---|---|---|
| F-C-01 | AI 辅助推荐与解释降级口径需稳定性复核 | C6、学生 AI 推荐页面 | 本期 C6 已接入；建议补充外部模型可用性与降级指标 |
