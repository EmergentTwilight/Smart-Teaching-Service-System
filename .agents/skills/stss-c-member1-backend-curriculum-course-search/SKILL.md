...
9. GET /offerings/available 的只读 eligibility 计算与展示原因，覆盖容量、已选、时间冲突、先修、培养方案适配、当前阶段等原因。
...
// /offerings/available 可以返回只读 eligibility 和 reasons，包括容量、已选、Schedule 冲突、
// 先修、培养方案适配和当前阶段。最终选课成功、容量扣减、重复选课和 max_credits
// 校验仍由成员 2 在 enrollment.service.ts 事务内实现。
```
