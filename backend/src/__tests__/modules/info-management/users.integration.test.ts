...
async function cleanupDatabase() {
  ...
  await prisma.user.deleteMany({
    where: {
      OR: [{ username: { startsWith: 'itest_usvc_' } }, { email: { startsWith: 'itest_usvc_' } }],
    },
  })

  // 删除测试创建的角色和权限
  await prisma.rolePermission.deleteMany({
    where: { role: { code: { startsWith: 'itest_' } } },
  })
  await prisma.permission.deleteMany({
    where: { code: { startsWith: 'itest_' } },
  })
...
async function createTestUser(overrides: Record<string, unknown> = {}) {
  const username = `itest_usvc_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`
  ...
  const user = await prisma.user.create({
    data: {
      ...
      email: `itest_usvc_${Date.now()}@test.com`,
      realName: '测试用户',
      ...overrides,
    },
  })
...
describe('UsersService Integration Tests', () => {
  ...
  describe('用户 CRUD', () => {
    describe('createUser', () => {
      it('应该成功创建用户并写入数据库', async () => {
        ...
        expect(dbUser!.username).toBe('itest_usvc_newuser')
        expect(dbUser!.email).toBe('itest_usvc_newuser@test.com')
      ...
      it('邮箱冲突时应该抛出 ConflictError', async () => {
        // 先创建一个用户
        await createTestUser({ email: 'itest_usvc_conflict_email@test.com' })
    ...
    describe('getUsers', () => {
      it('应该成功查询用户列表并验证分页功能', async () => {
        ...
        for (let i = 0; i < 15; i++) {
          await createTestUser({
            username: `itest_usvc_list_${i}`,
            email: `itest_usvc_list_${i}@test.com`,
            realName: `列表用户${i}`,
          })
        }
      ...
      it('应该支持按关键词搜索', async () => {
        // 创建特定用户
        await createTestUser({
          username: 'itest_usvc_searchuser',
          realName: '搜索测试用户',
          email: 'itest_usvc_searchuser@test.com',
        })
        ...
        expect(result.items.some((u) => u.username === 'itest_usvc_searchuser')).toBe(true)
      })

      it('应该支持按状态筛选', async () => {
        // 创建不同状态的用户
        await createTestUser({
          username: 'itest_usvc_active',
          status: 'ACTIVE',
        })
        await createTestUser({
          username: 'itest_usvc_inactive',
          status: 'INACTIVE',
        })
    ...
    describe('updateUser', () => {
      it('应该成功更新用户并验证数据库中字段已更新', async () => {
        const user = await createTestUser({
          username: 'itest_usvc_update',
          realName: '更新前',
          email: 'itest_usvc_update@test.com',
        })
      ...
      it('更新邮箱时应该检查唯一性', async () => {
        const user1 = await createTestUser({
          username: 'itest_usvc_update1',
          email: 'itest_usvc_update1@test.com',
        })
        await createTestUser({
          username: 'itest_usvc_update2',
          email: 'itest_usvc_update2@test.com',
        })

        await expect(
          usersService.updateUser(user1.id, { email: 'itest_usvc_update2@test.com' })
        ).rejects.toBeInstanceOf(ConflictError)
      })
    })
  ...
  describe('密码管理', () => {
    describe('changePassword', () => {
      ...
      it('修改密码后应该吊销所有 Refresh Token', async () => {
        const user = await createTestUser({
          username: 'itest_usvc_revoketoken',
          passwordHash: await hashPassword('OldPassword123'),
        })
    ...
    describe('resetPassword', () => {
      ...
      it('重置密码后应该吊销所有 Refresh Token', async () => {
        const user = await createTestUser({
          username: 'itest_usvc_resetrevoke',
          passwordHash: await hashPassword('OldPassword123'),
        })
  ...
  describe('角色管理', () => {
    describe('assignRoles', () => {
      it('应该成功分配角色并验证 user_roles 表中有对应记录', async () => {
        const user = await createTestUser({
          username: 'itest_usvc_assignrole',
        })
      ...
      it('角色不存在时应该抛出 NotFoundError', async () => {
        const user = await createTestUser({
          username: 'itest_usvc_invalidrole',
        })
      ...
      it('分配角色后应该吊销所有 Refresh Token', async () => {
        const user = await createTestUser({
          username: 'itest_usvc_rolerevoke',
        })
      ...
      it('重复分配角色应该替换原有角色', async () => {
        const user = await createTestUser({
          username: 'itest_usvc_replacerole',
        })
    ...
    describe('revokeRole', () => {
      it('应该成功撤销角色并验证 user_roles 表中记录已删除', async () => {
        const user = await createTestUser({
          username: 'itest_usvc_revokerole',
        })
      ...
      it('用户未分配该角色时应该抛出 NotFoundError', async () => {
        const user = await createTestUser({
          username: 'itest_usvc_notassigned',
        })
      ...
      it('撤销角色后应该吊销所有 Refresh Token', async () => {
        const user = await createTestUser({
          username: 'itest_usvc_revoketokenrole',
        })
  ...
  describe('批量操作', () => {
    describe('batchCreateUsers', () => {
      it('应该成功批量创建用户并验证所有用户都已写入数据库', async () => {
        const users = [
          {
            username: 'itest_usvc_batch1',
            password: 'Password123',
            realName: '批量用户1',
            email: 'itest_usvc_batch1@test.com',
            roleIds: [testRoleIds[0]],
          },
          {
            username: 'itest_usvc_batch2',
            password: 'Password456',
            realName: '批量用户2',
            email: 'itest_usvc_batch2@test.com',
            roleIds: [testRoleIds[1]],
          },
          {
            username: 'itest_usvc_batch3',
            password: 'Password789',
            realName: '批量用户3',
            email: 'itest_usvc_batch3@test.com',
          },
        ]
      ...
      it('用户名冲突时应该抛出 ConflictError 并回滚所有操作', async () => {
        ...
        const users = [
          {
            username: 'itest_usvc_batchnew1',
            password: 'Password123',
            realName: '新用户1',
          },
          {
            username: 'itest_usvc_batchconflict', // 冲突
            password: 'Password456',
            realName: '冲突用户',
          },
        ]
      ...
      it('超过100个用户时应该抛出 ValidationError', async () => {
        const users = Array(101)
          .fill(null)
          .map((_, i) => ({
            username: `itest_usvc_toomany_${i}`,
            password: 'Password123',
            realName: `用户${i}`,
          }))
    ...
    describe('batchUpdateStatus', () => {
      ...
      it('不存在的用户应该抛出 NotFoundError', async () => {
        const user = await createTestUser({ username: 'itest_usvc_existing' })
  ...
  describe('getUserById', () => {
    it('应该成功获取用户详情', async () => {
      const user = await createTestUser({
        username: 'itest_usvc_getbyid',
        email: 'itest_usvc_getbyid@test.com',
        realName: '详情用户',
      })
  ...
  describe('updateStatus', () => {
    it('应该成功修改用户状态', async () => {
      const user = await createTestUser({
        username: 'itest_usvc_updatestatus',
        status: 'ACTIVE',
      })
  ...
  describe('getUserPermissions', () => {
    it('应该成功获取用户权限列表', async () => {
      ...
      const user = await createTestUser({
        username: 'itest_usvc_permissions',
      })
