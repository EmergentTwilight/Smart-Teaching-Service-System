...
export const buildStrategyPrompt = ({
  ...
  const payload = {
    constraints: {
      ...
      naturalLanguagePreference: profile.naturalLanguagePreference,
      fallbackContext,
    },
    allowed_course_offering_ids: candidates.map((item) => item.id),
  ...
  return [
    ...
    '输入为 candidate_pool（已通过硬规则），每个 recommendationIds 元素必须逐字复制 allowed_course_offering_ids 中的值。',
    '输出要求：严格 JSON，不要 Markdown。',
    '输出 JSON 字段：',
    '- plans: 数组，每项必须包含 id、title、rationale、recommendationIds、riskLevel。',
    '- id 只能取 balanced、required_first、low_risk。',
    '- recommendationIds 是字符串数组，只能使用 allowed_course_offering_ids 中的真实 ID，不要使用示例 ID 或自造 ID。',
    '- recommendationSummary: 中文总体说明。',
    '补充规则：',
