...
培养方案制定功能用于支持学生在选课前理解所属专业的学分要求和课程结构，并据此确认当前培养方案作为正式选课依据。系统应以培养方案、培养方案课程和课程类型为基础展示专业必修课、专业选修课和公共课等课程类别，不应由 C 模块自行定义与 A 模块冲突的课程主数据。v2.0 验收基线要求培养方案确认结果可供选课流程校验；确认结果应持久化为学生与培养方案的确认记录，且确认时间早于培养方案更新时间时应要求重新确认。公共课仅按课程类型统计已选学分。
