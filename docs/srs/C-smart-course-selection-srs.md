...
- 培养方案查看与选课计划：展示学生所属专业培养方案、课程分类、总学分、必修/选修学分要求和建议修读学期，支持学生据此制定选课计划；C 子系统不持久化学生对培养方案的确认状态。
...
| ---- | -------- | ---------- | -------- | ---------- | ---------- |
| `US-C-01` | 学生查看培养方案并制定计划 | 学生 | 学生已登录；学生所属专业和年级存在对应培养方案 | 系统展示培养方案中的课程分类、学分要求和建议修读学期，学生据此制定拟修课程计划后进入选课流程 | 缺少培养方案时，应提示无法生成可选课程列表并联系教务管理人员。 |
...
| `US-C-04` | 系统自动显示可选课程 | 学生 | 学生所属专业和年级存在可匹配培养方案；当前存在启用的选课时间段 | 系统根据培养方案、课程开设状态、容量和选课阶段生成可选课程列表 | 若无可选课程，系统应显示原因而不是空白页面。 |
...
培养方案制定功能用于支持学生在选课前理解所属专业的学分要求和课程结构，并据此形成选课计划。系统应以培养方案、培养方案课程和课程类型为基础展示专业必修课、专业选修课和公共课等课程类别，不应由 C 模块自行定义与 A 模块冲突的课程主数据。现有数据库设计不包含学生培养方案确认记录，也不包含公共课独立最低学分字段，因此 C 子系统不要求持久化确认状态，不以“已确认培养方案”作为选课阻断条件；公共课仅按课程类型统计已选学分。
...
| `FR-C-04` | 学生进入正式选课前，系统应提供培养方案查看与选课计划入口，使学生能够基于相关因素制定个人选课计划；该入口不要求保存学生确认状态。 | Must |
| `FR-C-05` | 系统应根据培养方案、已选课程和课程学分统计学生当前选课计划的学分构成，包括总学分、必修课学分、选修课学分和公共课已选学分；公共课不设置独立最低学分达标判断。 | Must |
...
| 编号 | 覆盖需求 | 验证标准 |
| ---- | -------- | -------- |
| `VC-C-01` | `FR-C-01` 至 `FR-C-07` | 使用具有专业和年级信息的学生账号进入培养方案页面，系统能展示对应培养方案、学分要求、课程分类和建议修读学期；页面不要求保存培养方案确认状态；公共课只验证已选学分统计，不验证独立最低学分目标；缺少培养方案时给出明确提示并阻止生成可选课程列表。 |
