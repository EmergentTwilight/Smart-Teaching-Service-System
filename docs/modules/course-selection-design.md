...
1. `docs/srs/C-smart-course-selection-srs.md`，即 C 组 SRS。
2. `docs/database-design.md` 和 Prisma schema 中已有数据模型。
3. `docs/development-specifications.md` 中的项目结构、分支和代码规范。
4. 当前仓库中 A/B/D/F 组已形成的模块风格。
5. coding agent 的建议和自动生成结果。

如 SRS 中的需求编号与本文 TODO 编号不一致，以 SRS 为准，但业务边界和硬性规则不得改变。
...
教师只能查看或导出本人任课课程的学生名单。管理员可在授权范围内查看和导出。导出内容应来自有效选课记录，不能从前端缓存生成。
...
| 教师查看名单 | 只能查看本人任课课程，管理员除外。 |
| 教务管理阶段 | 仅教务管理角色或系统管理员可操作。 |
| 手动加课 | 仅教务管理角色或系统管理员可操作，并记录原因。 |
| AI 辅助 | 仅使用当前用户可见数据生成建议。 |

具体角色名应与现有项目保持一致，例如 `student`、`teacher`、`admin`、`super_admin` 或仓库中实际角色枚举。
