...
```text
联调分支：integration/C-intergroup-0615
基线分支：origin/develop @ 87338ab
C 组分支：origin/dev/C @ 924bff3
合并方式：从 origin/develop 新建联调分支后 merge origin/dev/C
```

合并处理：

```text
1. .gitignore：合并 develop 侧本地工具忽略项与 C 侧 .history 忽略项。
2. backend/src/app.ts：同时保留 A/B/F 已在 develop 的路由挂载，并新增 /api/v1/course-selection。
3. pnpm-lock.yaml：先使用 develop 侧 lockfile 作为基线，再通过 Docker wrapper 按合并后的 package.json 重新生成。
4. backend/src/modules/course-selection/selection-period.controller.ts：修复 C5 listPeriods 分页响应 meta 的 pageSize/totalPages 到 page_size/total_pages 契约转换，仅为类型与 API 契约对齐，不改变业务逻辑。
```
...
postgres：Up / healthy
redis：Up / healthy
server：Up，端口 localhost:3000
web：Up，端口 localhost:5173
```

环境准备命令和结果：

```bash
./scripts/codex-docker-run.sh 'pnpm install --lockfile-only'
```

结果：通过。实际 service 为 `server`，容器工作目录为 `/app`。

```bash
./scripts/codex-docker-run.sh 'pnpm install --frozen-lockfile && pnpm --filter @stss/server db:generate'
```

结果：通过。实际 service 为 `server`，容器工作目录为 `/app`。

```bash
CODEX_DOCKER_SERVICE=web CODEX_DOCKER_WORKDIR=/app ./scripts/codex-docker-run.sh 'pnpm install --frozen-lockfile'
```

结果：通过。实际 service 为 `web`，容器工作目录为 `/app`。
...
```text
1. 已从 develop 创建 C 组专用组间联调分支。
2. 已合入 C 组 dev/C。
3. 已解决合并冲突。
4. 已完成后端 typecheck 基础校验。
5. 已完成前端 typecheck 基础校验。
```
...
```text
暂无基础校验阻塞失败。
首次后端 typecheck 曾因容器依赖未同步和 Prisma Client 未重新生成失败；已通过 install + db:generate 清除环境问题。
首次前端 typecheck 曾因 web 容器依赖未同步导致 ahooks 缺失；已通过 web 服务内 frozen install 清除环境问题。
```
...
```text
基础校验阶段暂无 P0/P1 阻塞。
后续仍需执行 A/B/F/C 端到端手动流程后，才能建议合入 develop。
```

非阻塞 TODO：

```text
1. C6 后端暂时降级：AI 推荐只允许 fallback 或不可用提示，不得影响普通选课，不得写 Enrollment。
2. 需要继续准备 A/B/F/C 一致的组间联调数据，包括账号、学生/教师关系、专业、课程、开课、排课、培养方案、先修与成绩数据。
```

是否允许合入 dev/C：

```text
本轮操作不是组员分支合入 dev/C，而是 dev/C 合入组间联调分支。
```

是否允许从 dev/C 合入上级分支：

```text
暂不直接合入 develop。当前结论是：C 组代码已可进入组间联调分支继续做 A/B/F/C 端到端验证。
```

需要其他组确认的问题：

```text
1. A 组确认 student/teacher/admin 账号与 Student/Teacher/Admin 关联数据。
2. B 组确认 Course/CourseOffering/Schedule 字段与 C 组冲突判断口径。
3. F 组确认已修课程、成绩通过状态和先修课检查口径。
```
